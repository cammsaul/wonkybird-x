...
Pipe::Pipe(unsigned size, bool upsideDown):
	...
	layer_		(layerNamed(layerName_.c_str()))
{
...
const CCSize& Pipe::getContentSize() const {
	CCSize size = {layer_->getContentSize().width, layer_->getContentSize().height};
...
void Pipe::setPosition(const CCPoint& pos) {
